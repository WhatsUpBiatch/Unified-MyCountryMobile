...
Design notes:
  ...
  * A leg with a bridge_uuid whose partner hasn't arrived yet is inserted solo
    (with b_leg_uuid pointing at the expected partner) so the call is never
    invisible while waiting; when the partner file does arrive, it merges its
    facts into that same row instead of inserting a second one. b_leg_uuid is
    an existing, previously-unused column — no schema change.
...
def parse(path):
    ...
    SIP hosts) needed to classify and merge the pair. Extra keys are simply
    ignored by the COLS-driven insert/update."""
    ...
    return {
        ...
        "b_leg_uuid": None,
        # not DB columns - used only for leg classification during merge
...
def merge_fields(carrier, user):
    ...
    bridged call into the single row the UI should show."""
    ...
    for path in files:
        ...
        with conn.cursor() as cur:
            ...
            if row["bridge_uuid"]:
                ...
                if partner_row:
                    partner_path = find_leg_file(row["bridge_uuid"])
                    if partner_path:
                        try:
                ...
                if {role_incoming, role_partner} == {"carrier", "user"}:
                    ...
                    if apply_:
                        ...
                        cur.execute(
                            ...
                            [merged[c] for c in MERGE_COLS] + [row["xml_cdr_uuid"], row["bridge_uuid"]],
                        )
                        os.makedirs(DONE_DIR, exist_ok=True)
                        shutil.move(path, os.path.join(DONE_DIR, os.path.basename(path)))
                    stats["merged"] += 1
            ...
            if apply_:
                row["b_leg_uuid"] = cut(row["bridge_uuid"], 36)
