#!/usr/bin/env bash
# Bundle the pure scope helper to CommonJS and run the unit tests. No database.
#
#   bash backend-patches/admin-scope/tests/run.sh
#
...
BUILD_DIR="${BUILD_DIR:-$(mktemp -d /tmp/admin-scope-tests.XXXXXX)}"

HELPERS=(
  "$API/src/helpers/adminScope.ts"
