 ...
 * request. THIS FILE IS A PORT, NOT THE SOURCE. The source is
 * `src/lib/ip-allowlist.ts` in the frontend repo - read that file's header for
 * the full design notes (IPv4 + IPv6 CIDR matching, break-glass, legacy data).
 *
 * `matches`, `isAllowed` and `evaluateIpAllowlist` must stay byte-identical to
 * the frontend copy; `scripts/verify-ip-allowlist.mjs` in the frontend repo
 * checks for their presence on every run, and a manual diff is the way to
 * confirm they still MATCH, not just exist. The frontend needs its own copy
 * because it ships in the browser and enforcement has to run without it; this
 * copy is what a request actually gets checked against.
 */
