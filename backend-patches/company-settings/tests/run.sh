...
#
#   bash backend-patches/company-settings/tests/run.sh
#
...
BUILD_DIR="${BUILD_DIR:-$(mktemp -d /tmp/company-settings-tests.XXXXXX)}"

[ -x "$TSC" ] || { echo "tsc not found at $TSC"; exit 1; }

"$TSC" \
  "$REPOS/tenant-api/src/helpers/companyRuleFlags.ts" \
  "$REPOS/tenant-api/src/helpers/companySettingsSections.ts" \
