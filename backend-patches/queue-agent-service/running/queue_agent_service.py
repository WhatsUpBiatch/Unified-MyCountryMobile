...
from queue_exit import queue_exit, queue_timeout_seconds, max_callers
...
class QueueAgentHandler(BaseHTTPRequestHandler):
    ...
    def _queue_agents(self, queue_key, query):
        ...
        # What this queue was told to do when nobody picks up. Saved by the
        # product all along and read by nothing until now, so a caller waited a
        # hardcoded sixty seconds and was then hung up on whatever the queue said.
        #
        # Sent on every answer rather than only when the list is empty: the
        # switch has to know how long to wait BEFORE it starts waiting, and it
        # asks this service once at the start of the call.
        exit_to = queue_exit(queue)

        payload = {
            "agents": [agent_payload(r) for r in chosen],
            "queue_timeout": queue_timeout_seconds(queue),
            "max_callers": max_callers(queue),
            "exit": exit_to,
            "queue": queue_key,
            "strategy": normalize_strategy(strategy),
